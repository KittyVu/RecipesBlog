...
export default function CateCake() {
  const id = 3;
  return (
    <div className="m-[2em]">
      <h1
      ...
      <div className="flex flex-col lg:flex-row gap-[2em] justify-between p-4">

        {/* Left column (big featured cake) */}
        <div className="flex-1">
          <Link href={`/recipes/${id}`}><img src="/cate-cake.png" alt="featured cake" className="w-full" /></Link>
          <div className="flex flex-col sm:flex-row gap-[1em] items-center justify-between">
            <Link href={`/recipes/${id}`}><h1 className={`text-2xl md:text-3xl font-extrabold m-4 ${playfair2.className}`}>Strawberry Cheesecake</h1></Link>
        ...
        {/* Right column (list of smaller cakes) */}
        <div className="flex flex-col gap-[2em] flex-1">
          {[
            { img: "cate-cake1.png", title: "Caramel Cake" },
            { img: "cate-cake2.png", title: "Fig Cake" },
            { img: "cate-cake3.png", title: "Lemon Cake" },
          ].map((item, i) => (
